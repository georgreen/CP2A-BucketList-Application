...
class ItemOperations(BaseResource):
    ...
    def patch(self, bucket_id, item_id):
        ...
        # get item
        item = self.profile.get_item(item_id=item_id)
    ...
    def put(self, bucket_id, item_id):
        ...
        # editted
        edit = self.profile.edit_asset(
            item=True, name=name, description=description, asset_id=item_id)
        if not edit:
            return {"message": "Unable to edit item"}, 500
    ...
    def delete(self, bucket_id, item_id):
        ...
        deleted_item = self.profile.delete_asset(item=True, asset_id=item_id)
        if not deleted_item:
